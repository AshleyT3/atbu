...
from atbu.common.exception import AlreadyUsedError
from atbu.common.aes_cbc import AesCbcPaddingEncryptor

CHUNK_READER_CB_ALLBYTES = "all-bytes"
class ChunkReaderCbCode(Enum):
    AllBytes = 1
    CipertextBytes = 2
...
class ChunkSizeFileReader:
    ...
    def __init__(
        ...
        user_func: Callable[[ChunkReaderCbCode, bytes], None],
    ):
    ...
    def queue_data(self, bytes_to_queue, do_not_encrypt: bool = True):
        if not do_not_encrypt:
            raise ValueError(
                f"ChunkSizeFileReader: Encryption is not active, "
                f"do_not_encrypt=False is disallowed."
            )
        self._checkused()
        if self._eof_detected:
            raise EOFError(
    ...
    def read_chunk(self):
        ...
        self._call_user_func(ChunkReaderCbCode.AllBytes, bytes_to_return)
        return bytes_to_return

    def read(self, size):
        if size != self._chunk_size:
            raise Exception(
                f"You are only allowed to read chunk sizes of "
                f"{self._chunk_size} bytes from this instance."
            )
...
class ChunkSizeEncryptorReader(ChunkSizeFileReader):
    ...
    def queue_data(self, bytes_to_queue, do_not_encrypt: bool = False):
        ...
        if do_not_encrypt:
            ...
            if len(new_cipher_text) > 0:
                self._call_user_func(ChunkReaderCbCode.CipertextBytes, new_cipher_text)
                if len(self._pending_output) == 0:
    ...
    def read_chunk(self):
        ...
        if self.encryptor.is_finalized:
            ...
            self._call_user_func(ChunkReaderCbCode.AllBytes, ciphertext_chunk_to_return)
